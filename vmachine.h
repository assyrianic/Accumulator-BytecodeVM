...
#define VMACHINE_INSTR_SET \
	X(halt) X(nop) \
    X(imm) X(inc) X(dec) X(zero) X(lea) X(pushlr) X(poplr) \
    X(add) X(vadd) X(sub) X(mul) X(divi) X(mod) \
...
#define INSTR_PARAMS    union VMachinePtr pc, union VMachineVal rA, uintptr_t sp, uintptr_t fp, uintptr_t lr, struct VMachineState *state
#define INSTR_ARGS      pc, rA, sp, fp, lr, state
